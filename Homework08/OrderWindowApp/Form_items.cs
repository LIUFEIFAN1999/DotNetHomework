    ...
    public partial class Form_items : Form
    {
        ...
        private void button_createOrder_Click(object sender, EventArgs e)
        {
            ...
            for(int index = 0; index < n; index++)
            {
                ...
                {
                    order.ItemList.Add(itemList[index]);
                    itemList[index] = null;
                }
            }
            ...
            (this.MdiParent as Form_orderList).orderbindingSource.DataSource = null;
            (this.MdiParent as Form_orderList).orderbindingSource.DataSource = orderService.OrderList;
            if(button_createOrder.Text == "修改订单")
            {
                this.Dispose();
            }
